...
contract HtswapMaster is Ownable {
    ...
    uint256 public totalLockReward;
    ...
    event UnlockReward(address indexed customer, uint256 indexed pid, uint256 amount);
    event LockReward(address indexed customer, uint256 indexed pid, uint256 amount);
    ...
    function getPendingReward(uint256 _pid, address _customer) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 commReward = tokenReward.div(10);
            uint256 poolReward = tokenReward.sub(devReward).sub(commReward);
    ...
    function updatePoolStatus(uint256 _pid) public {
        ...
        if (tokenReward > 0 && lpSupply > 0) {
            ...
            rewardToken.mint(commRewardAddr, commReward);
    ...
    function _harvest(uint256 _pid, address _user, PoolStatus storage pool, CustomerStatus storage customer) internal {
            RewardLockStatus storage lockStat = rewardLockStatus[_pid][_user];
            uint256 harvest = 0;
            if (lockStat.lastUnlockTime == 0) {
                ...
                lockStat.lastUnlockTime = block.timestamp;
                if (harvest > 0) {
                    totalLockReward = totalLockReward.sub(harvest);
                    emit UnlockReward(_user, _pid, harvest);
                }
            }
                ...
                if(pending > 0) {
                    if (rewardLockPeriod > 0 && rewardLockPercent > 0) {
                        ...
                        if (locking > 0) {
                            ...
                            totalLockReward = totalLockReward.add(locking);
                            emit LockReward(_user, _pid, locking);
                        }
                    } else {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        _harvest(_pid, msg.sender, pool, customer);
        if(_amount > 0) {
